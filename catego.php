...
<div class="album py-5 bg-light">
    ...
    <table class="table">
            ...
            <tbody>
<?php foreach( (array) $produits as $produit): ?>
                <tr>
         ...
         /*  $req1=$access->prepare ("SELECT  classe FROM  categorie type WHERE idcategorie =:idcategorie");
           $req1->bindValue(":idcategorie",$produit->idcategorie,PDO::PARAM_INT);
            $req1->execute();
    
            $c = $req->fetchAll(PDO::FETCH_OBJ);*/
            $dab = new PDO("mysql:host=localhost;dbname=monoshop;", "root", "");
            $sql = $dab->prepare("SELECT  *  FROM categorie  where idcategorie=$produit->idcategorie ");
            $sql->execute();
            foreach ($sql as $res);
     
            ?>
                 <td style="font-weight: bold; color: red;"><?=   $res["classe"]; ?></td>
                <td><a href="editer.php?id=<?= $produit->id ?>"><i class="fa fa-pencil" style="font-size: 30px;"></i></a></td>
                <td><a href="supp1.php?id=<?= $produit->id ?>"><i class="btn btn-danger btn-sm" style="font-size: 30px;"></i></a></td>
                </tr>      
<?php endforeach; ?>
